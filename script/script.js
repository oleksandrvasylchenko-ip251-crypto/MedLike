...
function startTimer(phone) {
  ...
  document.getElementById("timerStatus").textContent =
    `⏳ Очікуємо дзвінка на номер ${phone}...`;
  ...
  timerInterval = setInterval(() => {
    ...
    if (timerSeconds <= 60) {
      ...
      document.getElementById("timerStatus").textContent =
        "⚠️ Скоро зателефонують!";
    } else if (timerSeconds <= 180) {
      timerDisplay.style.color = "#f39c12";
      document.getElementById("timerStatus").textContent =
        "⏳ Готуємось до дзвінка...";
    }
    ...
    if (timerSeconds <= 0) {
      ...
      document.getElementById("timerStatus").textContent =
        "✅ Дзвінок виконано! Дякуємо за довіру!";
...
function initMobileGallery() {
  ...
  if (!navContainer) {
    ...
    navContainer.innerHTML = `
        ...
        `;
    dotsContainer.parentNode.insertBefore(
      navContainer,
      dotsContainer.nextSibling,
    );
  }
